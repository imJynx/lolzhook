...
namespace Offset
{
    ...
    struct
    {
        DWORD BulletServices = 0x16B0; //NEW
        DWORD TotalHit = 0x40;
        ...
        DWORD iTeamNum = 0x3BF; //NEW
        DWORD CameraServices = 0x10E0; // m_pCameraServices
        DWORD iFov = 0x210; //NEW
        DWORD iFovStart = 0x214; //NEW
        DWORD fFlags = 0x1F8; //NEW
        DWORD bSpottedByMask = 0x19C0 + 0xC; // entitySpottedState + bSpottedByMask
    } Pawn;

    struct
    {
        DWORD dwPlantedC4 = 0x180EB48; //NEW
        DWORD RealTime = 0x00;
